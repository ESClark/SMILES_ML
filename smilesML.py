...
def selectFeatures(X, y, e=0.1):
    '''
    Given:
        X: Feature matrix
        y: Target vector
        e: Correlation error threshold
    Return:
        X_new: Subset of feature matrix
    '''
    data = pd.DataFrame(X)
    data['target'] = y
    corr = data.corr()
    ...
    plt.matshow(corr, cmap='coolwarm', vmin=-1, vmax=1, fignum=fig.number)
    plt.colorbar()
...
if __name__ == '__main__':
    ...
    for k in wavenumbers:  
        Freqs = preprocess_data(k, binsize)  # y target vector
        X, y = selectFeatures(featVecs, np.array(Freqs), 0.1)
        #X_train, X_test, y_train, y_test = train_test_split(featVecs, Freqs, test_size=0.2, random_state=MAGIC_NUMBER)
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=MAGIC_NUMBER)
    ...
    data['scores'] = scores
    data['prior_ratios'] = prior_ratios
    data['max_depth_list'] = max_depth_list
    data['min_samples_leaf_list'] = min_samples_leaf_list
    ...
    scores = data['scores']
    prior_ratios = data['prior_ratios']
    max_depth_list = data['max_depth_list']
    min_samples_leaf_list = data['min_samples_leaf_list']

    plt.plot(wavenumbers, scores, label='Decision Tree')
    plt.plot(wavenumbers, prior_ratios, label='Maximum class classifier')
    plt.xlabel('Wavenumbers')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig('featureSelection2.png')
    plt.show()
    '''
